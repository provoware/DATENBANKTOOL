...
_TOPICS = {
    ...
    "folders": _topic(
        "folders",
        "Ordnerübersicht",
        "Zeigt Ordnergrößen, Dateizahlen und größte Platzfresser; CSV ist für LibreOffice verfügbar.",
        "Fasst einen abgeschlossenen Scan einschließlich Unterordnern zusammen und kann die gefilterte Übersicht als JSON, CSV oder HTML speichern.",
        "Rein lesend; nur ausdrücklich gewählte Berichte werden geschrieben und nicht still überschrieben.",
        "Sehr gering.",
        "Wenn große oder unübersichtliche Ordner gefunden oder als Tabelle weiterverarbeitet werden sollen.",
        (
            "Elternordner enthalten bewusst die Werte ihrer Unterordner.",
            "Ohne --all-pages enthält ein Bericht nur die gewählte Seite.",
        ),
        (
            "Indexdatenbank wählen.",
            "Ampel immer zusammen mit Klartext und Begründung lesen.",
            "Eingerückte Platzfresser prüfen.",
            "Für eine vollständige Calc-Tabelle --csv ZIEL --all-pages verwenden.",
        ),
        "Eine stabile Ordnerliste oder vollständige CSV-Tabelle erscheint; es wird nichts aufgeräumt.",
        (
            "Ordnerzeilen nicht addieren; Unterordner sind bereits enthalten.",
            "Existiert der Bericht, neuen Namen wählen oder bewusst --overwrite-report nutzen.",
        ),
        "datenbanktool index folders index.sqlite3 --csv ordner.csv --all-pages",
        ("ordner", "größe", "platzfresser", "speicher", "csv", "libreoffice", "calc"),
    ),
    ...
    "acceptance": _topic(
        ...
        "Prüft Indexaufbau, vollständigen CSV-Export, Laufzeit, Python-Speicher und unveränderte Quelldaten mit festen Profilen.",
        "Schreibt ausschließlich neue synthetische Testdaten und Berichte in einen noch nicht vorhandenen Arbeitsordner.",
        "Gering; persönliche Dateien werden nicht verwendet, der Testbestand kann aber viele Dateien anlegen.",
        "Vor Releases oder nach Änderungen an Scan-, Ordner- oder Exportlogik.",
        (
            "Einen neuen Arbeitsordner auf einem Datenträger mit ausreichend freien Dateieinträgen wählen.",
            "quick nutzt 600, standard 10.000 und large 100.000 Dateien.",
            "Eine automatische Prüfung ersetzt keine reale Testperson.",
        ),
        (
            "Passendes Profil wählen.",
            "Neuen Arbeitsordner angeben.",
            "Automatischen JSON- und Markdown-Bericht prüfen.",
            "NOVICE_ACCEPTANCE_CHECKLIST.md durch eine reale unerfahrene Testperson ausfüllen lassen.",
        ),
        "Alle automatischen Kriterien sind bestanden; die reale Laienabnahme bleibt separat sichtbar.",
        (
            "Arbeitsordner existiert: einen neuen Pfad verwenden, es wird nichts überschrieben.",
            "Zeit- oder Speichergrenze verfehlt: Bericht nach langsamster Phase prüfen.",
            "Large-Profil nur auf einem dafür geeigneten lokalen System ausführen.",
        ),
        "datenbanktool acceptance --profile quick --workspace ./abnahme-quick",
        ("abnahme", "benchmark", "lasttest", "großbestand", "laientest", "performance"),
    ),
    "explain": _topic(
        "explain",
        "Funktionen erklären",
