...
def _database_checks(database_path: Path) -> list[DiagnosticCheck]:
    ...
    checks.append(
        DiagnosticCheck(
            "datenbankversion",
            "red" if version > SCHEMA_VERSION else "green",
            "Diese Indexdatei stammt aus einer neueren Tool-Version."
            if version > SCHEMA_VERSION
            else "Die gespeicherte Datenbankversion wird unterstützt.",
...
def run_diagnostics(database_path: Path | None = None) -> DiagnosticResult:
    ...
    previous = previous_unfinished_run(ignore_process_id=os.getpid())
    checks.append(
        DiagnosticCheck(
            "letzter lauf",
